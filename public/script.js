var previousVisit = function (bool){
  ...
  if (bool) {
    div.innerHTML = "<script type='text/javascript'>new JsDatePick({useMode:2,target:'dateField5',dateFormat:'%Y-%m-%d'});new JsDatePick({useMode:2,target:'dateField6',dateFormat:'%Y-%m-%d'});</script><input class='input-mini' id='dateField5' style='display: inline' type='number' name='Times'/> time Latest visit from <input style='dsplay:inline' type='text' id='dateField6' name='LastVisitBegin'/> to <input type='text' name='LastVisitEnd'/>"
  } else {
    div.innerHTML = "";
  }
}
...
var addDatePicker = function(fieldId) {
    if(!document.getElementById("datePicker" + fieldId)){
      var script = document.createElement("SCRIPT");
      script.id = "datePicker" + fieldId;
      script.type = "text/javascript";
      script.innerHTML = "new JsDatePick({ useMode:2,target:" + fieldId + ", dateFormat:'%Y-%m-%d'})";
